...
module tb_ascon_aead128_dec_ad_buffered;
  ...
  reg ciphertext_in_valid_i;
  wire ciphertext_in_ready_o;
  reg [127:0] ciphertext_in_block_i;

  wire plaintext_out_valid_o;
  reg plaintext_out_ready_i;
  wire [127:0] plaintext_out_block_o;
  wire [4:0] plaintext_out_bytes_o;

  wire auth_out_valid_o;
  reg auth_out_ready_i;
  wire auth_out_ok_o;

  wire ad_fifo_empty_o;
  wire ad_fifo_full_o;
  wire [2:0] ad_fifo_level_o;
  wire ciphertext_fifo_empty_o;
  wire ciphertext_fifo_full_o;
  wire [2:0] ciphertext_fifo_level_o;
  wire plaintext_fifo_empty_o;
  wire plaintext_fifo_full_o;
  wire [2:0] plaintext_fifo_level_o;
  ...
  ascon_aead128_dec_ad_buffered #(
    ...
    .CT_FIFO_DEPTH_LOG2(2),
    .PT_FIFO_DEPTH_LOG2(2)
  ) dut (
    .clk(clk),
    .rst_n(rst_n),
    .clear_i(clear_i),
    ...
    .ciphertext_in_valid_i(ciphertext_in_valid_i),
    .ciphertext_in_ready_o(ciphertext_in_ready_o),
    .ciphertext_in_block_i(ciphertext_in_block_i),
    .plaintext_out_valid_o(plaintext_out_valid_o),
    .plaintext_out_ready_i(plaintext_out_ready_i),
    .plaintext_out_block_o(plaintext_out_block_o),
    .plaintext_out_bytes_o(plaintext_out_bytes_o),
    .auth_out_valid_o(auth_out_valid_o),
    .auth_out_ready_i(auth_out_ready_i),
    .auth_out_ok_o(auth_out_ok_o),
    .ad_fifo_empty_o(ad_fifo_empty_o),
    .ad_fifo_full_o(ad_fifo_full_o),
    .ad_fifo_level_o(ad_fifo_level_o),
    .ciphertext_fifo_empty_o(ciphertext_fifo_empty_o),
    .ciphertext_fifo_full_o(ciphertext_fifo_full_o),
    .ciphertext_fifo_level_o(ciphertext_fifo_level_o),
    .plaintext_fifo_empty_o(plaintext_fifo_empty_o),
    .plaintext_fifo_full_o(plaintext_fifo_full_o),
    .plaintext_fifo_level_o(plaintext_fifo_level_o)
  );
  ...
  task reset_dut;
    begin
      ...
      ciphertext_in_valid_i <= 1'b0;
      ciphertext_in_block_i <= 128'd0;
      plaintext_out_ready_i <= 1'b0;
      auth_out_ready_i <= 1'b0;
  ...
  task push_ct_block;
    ...
    begin
      ...
      while (!fire) begin
        @(negedge clk);
        ciphertext_in_block_i = block;
        ciphertext_in_valid_i = 1'b1;
        #1;
        fire = ciphertext_in_valid_i && ciphertext_in_ready_o;
        @(posedge clk);
        #1;
      end
      @(negedge clk);
  ...
  task run_case;
    ...
    begin
      ...
      // Preload high-volume input streams before starting the job.  This
      // models DMA/FIFO filling and verifies that start_i is independent of the
      // actual block source timing.
      for (i = 0; i < ad_blocks; i = i + 1) begin
        push_ad_block(pick_ad(case_idx, i));
      end
      for (i = 0; i < msg_blocks; i = i + 1) begin
        push_ct_block(pick_ct(case_idx, i));
      end
      ...
      if (!start_ready_o) begin
        $display("FAIL buffered DEC case%0d RPC=%0d start_ready_o low", case_idx, RPC);
        failed = 1'b1;
      end
      @(posedge clk);
      #1;
      ...
      while (!(done_seen && plaintext_fifo_empty_o) && cycles < 10000) begin
        @(negedge clk);
        cycles = cycles + 1;
        plaintext_out_ready_i = ((cycles % 7) != 2) && ((cycles % 11) != 4);
        auth_out_ready_i = ((cycles % 5) != 3);
        #1;
        pt_fire = plaintext_out_valid_o && plaintext_out_ready_i;
        auth_fire = auth_out_valid_o && auth_out_ready_i;
        pt_sample = plaintext_out_block_o;
        pt_bytes_sample = plaintext_out_bytes_o;
        auth_ok_sample = auth_out_ok_o;
        @(posedge clk);
        #1;

        if (pt_fire) begin
          if (pt_recv_idx >= msg_blocks) begin
            $display("FAIL buffered DEC case%0d RPC=%0d unexpected plaintext %032x",
                     case_idx, RPC, pt_sample);
            failed = 1'b1;
          end else if (pt_sample !== pick_pt(case_idx, pt_recv_idx)) begin
            $display("FAIL buffered DEC case%0d RPC=%0d pt%0d got=%032x exp=%032x",
                     case_idx, RPC, pt_recv_idx, pt_sample,
                     pick_pt(case_idx, pt_recv_idx));
            failed = 1'b1;
          end else if (pt_bytes_sample !== block_bytes(msg_len, pt_recv_idx)) begin
            $display("FAIL buffered DEC case%0d RPC=%0d pt%0d bytes got=%0d exp=%0d",
                     case_idx, RPC, pt_recv_idx, pt_bytes_sample,
                     block_bytes(msg_len, pt_recv_idx));
            failed = 1'b1;
          end
        ...
        if (auth_fire) begin
          saw_auth = 1;
          if (auth_ok_sample !== !tamper_tag) begin
            $display("FAIL buffered DEC case%0d RPC=%0d auth_ok got=%0d exp=%0d tamper=%0d",
                     case_idx, RPC, auth_ok_sample, !tamper_tag, tamper_tag);
            failed = 1'b1;
          end
        end
      ...
      if (cycles >= 10000) begin
        $display("FAIL buffered DEC case%0d RPC=%0d timeout", case_idx, RPC);
        failed = 1'b1;
      end
      if (pt_recv_idx != msg_blocks) begin
        $display("FAIL buffered DEC case%0d RPC=%0d received %0d plaintext blocks, expected %0d",
                 case_idx, RPC, pt_recv_idx, msg_blocks);
        failed = 1'b1;
      end
      if (!saw_auth) begin
        $display("FAIL buffered DEC case%0d RPC=%0d missing auth verdict", case_idx, RPC);
        failed = 1'b1;
      end
      if (!ad_fifo_empty_o || !ciphertext_fifo_empty_o || !plaintext_fifo_empty_o) begin
        $display("FAIL buffered DEC case%0d RPC=%0d non-empty FIFO after job: ad=%0d ct=%0d pt=%0d",
                 case_idx, RPC, ad_fifo_level_o, ciphertext_fifo_level_o,
                 plaintext_fifo_level_o);
        failed = 1'b1;
      end
      ...
      if (failed) begin
        ...
        $display("PASS buffered DEC case%0d RPC=%0d ad_bytes=%0d msg_bytes=%0d ad_blocks=%0d msg_blocks=%0d tamper=%0d cycles=%0d",
                 case_idx, RPC, ad_len, msg_len, ad_blocks, msg_blocks, tamper_tag, cycles);
      end
    end
  endtask
  ...
  initial begin
    ...
    for (case_i = 0; case_i < 10; case_i = case_i + 1) begin
      run_case(case_i, 0);
    end
    run_case(9, 1);

    if (errors == 0) begin
      $display("ALL BUFFERED AEAD DECRYPT TESTS PASSED for RPC=%0d", RPC);
    end else begin
      $display("BUFFERED AEAD DECRYPT TESTS FAILED for RPC=%0d errors=%0d", RPC, errors);
      $fatal;
    end
