...
module tb_ascon_aead128_mmio32;
  ...
  task mmio_write;
    ...
    begin
      ...
      reg_wdata_i = 32'd0;
      if (!fired) begin
        $display("FAIL mmio write timeout addr=%02x data=%08x", addr, data);
        errors = errors + 1;
      end
    end
  endtask
  ...
  task mmio_read;
    ...
    begin
      ...
      reg_addr_i = 8'd0;
      if (!fired) begin
        $display("FAIL mmio read timeout addr=%02x", addr);
        errors = errors + 1;
      end
    end
  endtask
  ...
  function [31:0] select_word;
    ...
    begin
      case (idx)
        0: select_word = block[31:0];
        1: select_word = block[63:32];
        2: select_word = block[95:64];
    ...
    begin
      ...
      for (i = 0; i < words; i = i + 1) begin
        ...
        if (meta[2:0] !== expected_word_bytes(total_len, i)) begin
          $display("FAIL mmio mode=%0d RPC=%0d word%0d bytes got=%0d exp=%0d",
                   DECRYPT, RPC, i, meta[2:0], expected_word_bytes(total_len, i));
          failed = 1'b1;
        end
        if (meta[8] !== (i == words - 1)) begin
          $display("FAIL mmio mode=%0d RPC=%0d word%0d last got=%0d exp=%0d",
        ...
        if (got !== exp) begin
          $display("FAIL mmio mode=%0d RPC=%0d word%0d got=%08x exp=%08x",
  ...
  task wait_result;
    ...
    begin
      ...
      if (!status[4]) begin
        $display("FAIL mmio mode=%0d RPC=%0d timeout waiting result", DECRYPT, RPC);
        errors = errors + 1;
      end
    end
  endtask
    ...
    begin
      ...
      if (DECRYPT == 0) begin
        ...
        if (result_tag !== pick_tag(case_idx)) begin
          $display("FAIL mmio enc RPC=%0d case%0d tag got=%032x exp=%032x",
                   RPC, case_idx, result_tag, pick_tag(case_idx));
          failed = 1'b1;
        end
        if (status[5] !== 1'b1) begin
          $display("FAIL mmio enc RPC=%0d case%0d auth_ok low", RPC, case_idx);
          failed = 1'b1;
        end
      end else begin
        if (status[5] !== 1'b1) begin
          $display("FAIL mmio dec RPC=%0d case%0d auth failed", RPC, case_idx);
          failed = 1'b1;
        end
      end

      if (!irq_o) begin
        $display("FAIL mmio mode=%0d RPC=%0d case%0d irq low with pending result", DECRYPT, RPC, case_idx);
        failed = 1'b1;
      end
      ...
      if (status[4] !== 1'b0) begin
        $display("FAIL mmio mode=%0d RPC=%0d case%0d result_ack failed status=%08x", DECRYPT, RPC, case_idx, status);
        failed = 1'b1;
      end
      ...
      if (failed) begin
        ...
        $display("PASS mmio case%0d mode=%0d RPC=%0d", case_idx, DECRYPT, RPC);
      end
    end
  endtask
  ...
  initial begin
    ...
    if (errors == 0) begin
      $display("ALL MMIO32 AEAD WRAPPER TESTS PASSED mode=%0d RPC=%0d", DECRYPT, RPC);
      $finish;
    end else begin
      $display("MMIO32 AEAD WRAPPER TESTS FAILED mode=%0d RPC=%0d errors=%0d", DECRYPT, RPC, errors);
      $fatal;
    end
  end
endmodule
