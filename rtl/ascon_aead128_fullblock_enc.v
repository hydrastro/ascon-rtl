`timescale 1ns/1ps
// SPDX-License-Identifier: Apache-2.0
//
// Phase 2.1 Ascon-AEAD128 encryption skeleton.
//
// Scope of this module:
//   - Encryption only.
//   - No associated data.
//   - Message length is an integer number of 16-byte blocks.
//   - The required final empty-block padding is still applied.
//   - Inputs/outputs use internal Ascon 64-bit word order, not CPU byte order.
//
// Packing convention:
//   key_i[127:64]         = K0 = LOADBYTES(k,     8)
//   key_i[63:0]           = K1 = LOADBYTES(k + 8, 8)
//   nonce_i[127:64]       = N0 = LOADBYTES(n,     8)
//   nonce_i[63:0]         = N1 = LOADBYTES(n + 8, 8)
//   plaintext_block_i     = {M0, M1}
//   ciphertext_block_o    = {C0, C1}
//   tag_o                 = {T0, T1}
//
// A wrapper that accepts raw bytes from NEORV32, AXI, SLINK, or Tiny Tapeout IO
// must perform byte packing/unpacking outside this cryptographic core.

module ascon_aead128_fullblock_enc #(
  parameter integer ROUNDS_PER_CYCLE = 1
) (
  input  wire         clk,
  input  wire         rst_n,

  input  wire         start_i,
  input  wire [127:0] key_i,
  input  wire [127:0] nonce_i,
  input  wire [31:0]  msg_blocks_i,

  output wire         busy_o,
  output reg          done_o,

  output wire         plaintext_ready_o,
  input  wire         plaintext_valid_i,
  input  wire [127:0] plaintext_block_i,

  output reg          ciphertext_valid_o,
  input  wire         ciphertext_ready_i,
  output reg  [127:0] ciphertext_block_o,

  output reg          tag_valid_o,
  input  wire         tag_ready_i,
  output reg  [127:0] tag_o
);

  localparam [63:0] ASCON_128A_IV = 64'h00001000808c0001;
  localparam [63:0] ASCON_PAD0    = 64'h0000000000000001;
  localparam [63:0] ASCON_DSEP    = 64'h8000000000000000;

  localparam [3:0] ST_IDLE            = 4'd0;
  localparam [3:0] ST_INIT_WAIT       = 4'd1;
  localparam [3:0] ST_WAIT_PLAINTEXT  = 4'd2;
  localparam [3:0] ST_MSG_PERM_WAIT   = 4'd3;
  localparam [3:0] ST_DRAIN_CT        = 4'd4;
  localparam [3:0] ST_FINAL_WAIT      = 4'd5;
  localparam [3:0] ST_TAG_WAIT        = 4'd6;

  reg [3:0]   state_q;
  reg [319:0] ascon_state_q;
  reg [127:0] key_q;
  reg [31:0]  blocks_left_q;

  reg         perm_start_q;
  reg [3:0]   perm_rounds_q;
  reg [319:0] perm_state_i_q;
  wire        perm_busy_w;
  wire        perm_done_w;
  wire [319:0] perm_state_o_w;

  wire [63:0] k0_w = key_q[127:64];
  wire [63:0] k1_w = key_q[63:0];

  wire [63:0] s0_w = ascon_state_q[319:256];
  wire [63:0] s1_w = ascon_state_q[255:192];
  wire [63:0] s2_w = ascon_state_q[191:128];
  wire [63:0] s3_w = ascon_state_q[127:64];
  wire [63:0] s4_w = ascon_state_q[63:0];

  wire [63:0] m0_w = plaintext_block_i[127:64];
  wire [63:0] m1_w = plaintext_block_i[63:0];

  wire [63:0] c0_w = s0_w ^ m0_w;
  wire [63:0] c1_w = s1_w ^ m1_w;

  assign busy_o = (state_q != ST_IDLE) | perm_busy_w;
  assign plaintext_ready_o =
    (state_q == ST_WAIT_PLAINTEXT) && !ciphertext_valid_o;

  ascon_perm_unrolled #(
    .ROUNDS_PER_CYCLE(ROUNDS_PER_CYCLE)
  ) u_perm (
    .clk     (clk),
    .rst_n   (rst_n),
    .start_i (perm_start_q),
    .rounds_i(perm_rounds_q),
    .state_i (perm_state_i_q),
    .busy_o  (perm_busy_w),
    .done_o  (perm_done_w),
    .state_o (perm_state_o_w)
  );

  task start_perm;
    input [3:0] rounds;
    input [319:0] state_in;
    begin
      perm_rounds_q  <= rounds;
      perm_state_i_q <= state_in;
      perm_start_q   <= 1'b1;
    end
  endtask

  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      state_q              <= ST_IDLE;
      ascon_state_q        <= 320'd0;
      key_q                <= 128'd0;
      blocks_left_q        <= 32'd0;
      perm_start_q         <= 1'b0;
      perm_rounds_q        <= 4'd0;
      perm_state_i_q       <= 320'd0;
      done_o               <= 1'b0;
      ciphertext_valid_o   <= 1'b0;
      ciphertext_block_o   <= 128'd0;
      tag_valid_o          <= 1'b0;
      tag_o                <= 128'd0;
    end else begin
      perm_start_q <= 1'b0;
      done_o       <= 1'b0;

      if (ciphertext_valid_o && ciphertext_ready_i) begin
        ciphertext_valid_o <= 1'b0;
      end

      case (state_q)
        ST_IDLE: begin
          if (start_i) begin
            key_q         <= key_i;
            blocks_left_q <= msg_blocks_i;
            ascon_state_q <= {ASCON_128A_IV, key_i[127:64], key_i[63:0],
                              nonce_i[127:64], nonce_i[63:0]};
            start_perm(4'd12, {ASCON_128A_IV, key_i[127:64], key_i[63:0],
                               nonce_i[127:64], nonce_i[63:0]});
            state_q <= ST_INIT_WAIT;
          end
        end

        ST_INIT_WAIT: begin
          if (perm_done_w) begin
            // Finish initialization: S3 ^= K0, S4 ^= K1, then domain separation.
            ascon_state_q <= {perm_state_o_w[319:128],
                              perm_state_o_w[127:64] ^ k0_w,
                              (perm_state_o_w[63:0] ^ k1_w) ^ ASCON_DSEP};

            state_q <= (blocks_left_q == 32'd0) ? ST_DRAIN_CT : ST_WAIT_PLAINTEXT;
          end
        end

        ST_WAIT_PLAINTEXT: begin
          if (plaintext_valid_i && plaintext_ready_o) begin
            ciphertext_block_o <= {c0_w, c1_w};
            ciphertext_valid_o <= 1'b1;
            ascon_state_q      <= {c0_w, c1_w, s2_w, s3_w, s4_w};
            blocks_left_q      <= blocks_left_q - 32'd1;
            start_perm(4'd8, {c0_w, c1_w, s2_w, s3_w, s4_w});
            state_q <= ST_MSG_PERM_WAIT;
          end
        end

        ST_MSG_PERM_WAIT: begin
          if (perm_done_w) begin
            ascon_state_q <= perm_state_o_w;
            if (blocks_left_q == 32'd0) begin
              state_q <= ST_DRAIN_CT;
            end else begin
              state_q <= ST_WAIT_PLAINTEXT;
            end
          end
        end

        ST_DRAIN_CT: begin
          if (!ciphertext_valid_o) begin
            // Final empty-block padding for messages whose length is exactly a
            // multiple of the 16-byte rate: S0 ^= PAD(0), then keyed finalization.
            ascon_state_q <= {s0_w ^ ASCON_PAD0, s1_w, s2_w, s3_w, s4_w};
            start_perm(4'd12, {s0_w ^ ASCON_PAD0,
                               s1_w,
                               s2_w ^ k0_w,
                               s3_w ^ k1_w,
                               s4_w});
            state_q <= ST_FINAL_WAIT;
          end
        end

        ST_FINAL_WAIT: begin
          if (perm_done_w) begin
            ascon_state_q <= {perm_state_o_w[319:128],
                              perm_state_o_w[127:64] ^ k0_w,
                              perm_state_o_w[63:0] ^ k1_w};
            tag_o       <= {perm_state_o_w[127:64] ^ k0_w,
                            perm_state_o_w[63:0] ^ k1_w};
            tag_valid_o <= 1'b1;
            state_q     <= ST_TAG_WAIT;
          end
        end

        ST_TAG_WAIT: begin
          if (tag_valid_o && tag_ready_i) begin
            tag_valid_o <= 1'b0;
            done_o      <= 1'b1;
            state_q     <= ST_IDLE;
          end
        end

        default: begin
          state_q <= ST_IDLE;
        end
      endcase
    end
  end

endmodule
